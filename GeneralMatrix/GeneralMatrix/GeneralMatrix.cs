...
namespace DotNetMatrix
{
	...
	internal class Maths 
	{
		...
		/// <param name="a"></param>
		/// <param name="b"></param>
		/// <returns>double square root</returns>
	...
	public class GeneralMatrix : System.ICloneable, System.Runtime.Serialization.ISerializable, System.IDisposable
	{
        ...
        /// <summary>
        /// Returns General Matrix as an XML Document that could be consumed 
        /// by ASP.NET GridView. The xml will be of the type:
        /// <code>
        /// <![CDATA[ <data> <row><column0>3</column0><column1>2</column1></row>/<Data>]]>
        /// 
        /// </code>
        /// </summary>
        public virtual XmlDocument XMLDocument
        {
            get{
                ...
                for (int i = 0; i < this.ColumnDimension; i++)
                {
                    ...
                    {
                        valueEl = aDocument.CreateElement(string.Format("column{0}", j));
                        valueEl.InnerText = this.GetElement(i, j).ToString();

                        columnEl.AppendChild(valueEl);
                    }

                    mainElement.AppendChild(columnEl);
                }
            

            aDocument.AppendChild(mainElement);
            return aDocument;
            }
        }
